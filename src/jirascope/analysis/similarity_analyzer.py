...
from ..models import WorkItem, DuplicateCandidate, DuplicateReport, CoherenceAnalysis
...
class MultiLevelSimilarityDetector:
    ...
    def _analyze_similarity_reasons(self, original: WorkItem, similar_item: Dict, score: float) -> List[str]:
        ...
        if any(word in similar_item.get("summary", "").lower() 
               for word in original.summary.lower().split() if len(word) > 3):
            ...
            for i, item in enumerate(work_items):
                ...
                for similar in similar_items:
                    ...
                    candidate = DuplicateCandidate(
                        ...
                        similarity_reasons=self._analyze_similarity_reasons(item, similar["work_item"], similar["score"])
                    )
                ...
                if (i + 1) % 10 == 0:
                    logger.debug(f"Processed {i + 1}/{len(work_items)} items for duplicate detection")
...
class SimilarityAnalyzer:
    ...
    async def find_potential_duplicates(
        ...
        try:
            ...
            if logger.cost_tracker:
                logger.log_cost("similarity", "duplicate_detection", estimated_cost, {
    ...
    async def hierarchical_coherence(self, epic_key: str) -> CoherenceAnalysis:
        ...
        try:
            ...
            outlier_threshold = min(0.4, sorted([oc["avg_similarity"] for oc in outlier_candidates])[:len(outlier_candidates)//5][-1] if outlier_candidates else 0.4)
            outlier_items = [oc["key"] for oc in outlier_candidates if oc["avg_similarity"] < outlier_threshold]
            
            # Generate recommendations
            recommendations = []
            if overall_coherence < 0.6:
                recommendations.append("Epic has low coherence - consider splitting or reorganizing")
            if len(outlier_items) > 0:
                recommendations.append(f"Review {len(outlier_items)} outlier items for correct Epic assignment")
            ...
            processing_time = time.time() - start_time
            estimated_cost = len(embeddings) * 0.0001  # Cost for similarity calculations
