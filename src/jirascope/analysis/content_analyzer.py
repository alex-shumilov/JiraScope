...
from ..models import WorkItem, QualityAnalysis, SplitAnalysis, SplitSuggestion, BatchAnalysisResult
...
class BatchContentAnalyzer:
    ...
    async def _process_batch(self, work_items: List[WorkItem], analysis_types: List[str]) -> Dict[str, Any]:
...
"""
        ...
        response = await self.claude_client.analyze(
            prompt=prompt,
            analysis_type="batch_quality_analysis"
        )
