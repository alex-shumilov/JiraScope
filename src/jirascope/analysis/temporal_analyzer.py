...
class ScopeDriftDetector:
    ...
    async def analyze_description_evolution(self, work_item_key: str) -> ScopeDriftAnalysis:
        ...
        try:
            ...
            description_changes = [
                change for change in change_history if "description" in change.get("fields", {})
            ]
            ...
            for i in range(1, len(description_changes)):
                ...
                if similarity < 0.7:  # Significant change threshold
                    ...
                    drift_event = ScopeDriftEvent(
                        ...
                        description=change_analysis.get(
                            "summary", "Significant scope change detected"
                        ),
    ...
    "summary": "brief description of the change"
        ...
        try:
            response = await self.claude_client.analyze(
                prompt=prompt, analysis_type="scope_change_analysis"
            )
...
class TemporalAnalyzer:
    ...
    async def detect_scope_drift_for_project(
        self, project_key: str, start_date=None, end_date=None
    ) -> BatchAnalysisResult:
        ...
        try:
            work_items = await self.drift_detector.mcp_client.get_work_items(
                project_key, start_date=start_date, end_date=end_date
            )
            ...
            async with self.drift_detector:
                for work_item in work_items:
                    try:
                        analysis = await self.drift_detector.analyze_description_evolution(
                            work_item.key
                        )
