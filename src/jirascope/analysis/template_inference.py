...
from ..models import WorkItem, TemplateInference
...
class TemplateInferenceEngine:
    ...
    async def infer_templates_from_samples(
        self, 
        issue_type: str, 
        high_quality_samples: List[WorkItem]
    ) -> TemplateInference:
        """Generate templates from high-quality work item examples."""
        ...
        if not self.claude_client:
            raise RuntimeError("TemplateInferenceEngine not initialized. Use async context manager.")
...
{{
    ...
    "template_notes": "explanation of patterns found"
            ...
            response = await self.claude_client.analyze(
                prompt=prompt,
                analysis_type="template_inference"
            )
            ...
            template = TemplateInference(
                ...
                title_template=template_data.get("title_template", f"{issue_type}: {{description}}"),
                description_template=template_data.get("description_template", "## Description\n\n{{description}}\n\n## Acceptance Criteria\n\n- [ ] {{criteria}}"),
    ...
    async def infer_multiple_templates(
        self, 
        samples_by_type: Dict[str, List[WorkItem]]
        ...
        for issue_type, samples in samples_by_type.items():
            if len(samples) >= 3:  # Minimum samples required
                ...
                logger.warning(f"Insufficient samples for {issue_type}: {len(samples)} (need at least 3)")
...
{implementation_notes}"""
        ...
        return {
            ...
            "description_template": description_templates.get(issue_type, "## Description\n\n{description}\n\n## Requirements\n\n{requirements}"),
    ...
    def validate_template_quality(self, template: TemplateInference) -> Dict[str, any]:
        ...
        if len(template.required_fields) < 2:
            validation_results["suggestions"].append("Consider more required fields for completeness")
        
        # Check confidence score
        if template.confidence_score < 0.6:
            validation_results["suggestions"].append("Low confidence score - consider adding more samples")
