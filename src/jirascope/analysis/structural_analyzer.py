...
from ..models import LabelingAnalysis, TechDebtCluster, TechDebtReport, WorkItem
...
class TechDebtClusterer:
    ...
    def _identify_tech_debt_items(self, work_items: list[WorkItem]) -> list[WorkItem]:
        ...
        for item in work_items:
            ...
            if any(
                keyword in title_lower or keyword in desc_lower for keyword in tech_debt_keywords
            ):
    ...
    def _estimate_effort_from_description(self, description: str) -> str:
        ...
        medium_indicators = ["refactor", "update", "all", "module", "library"]
    ...
    async def cluster_tech_debt_items(self, project_key: str | None = None) -> TechDebtReport:
        ...
        try:
            ...
            if len(tech_debt_items) < 2:
                ...
                return TechDebtReport(total_tech_debt_items=len(tech_debt_items))
            ...
            for cluster_id, items in clusters_dict.items():
                ...
                cluster = TechDebtCluster(
                    ...
                    impact_assessment=analysis.get(
                        "impact_assessment", "Medium impact if not addressed"
                    ),
                    recommended_approach=analysis.get(
    ...
    async def _find_tech_debt_items(self, project_key: str | None = None) -> list[dict[str, Any]]:
        ...
        for keyword in tech_debt_keywords:
            ...
            similar_items = await self.qdrant_client.search_similar_work_items(
                embeddings[0], limit=50, score_threshold=0.6
            )
    ...
    "recommended_approach": "approach to address this cluster"
        ...
        try:
            response = await self.claude_client.analyze(
                prompt=prompt, analysis_type="tech_debt_cluster_analysis"
            )
...
class StructuralAnalyzer:
    ...
    async def analyze_labeling_patterns(self, project_key: str | None = None) -> LabelingAnalysis:
        ...
        try:
            ...
            suggestions = self._generate_labeling_suggestions(label_counter, component_counter)
        ...
        async with self.tech_debt_clusterer:
            ...
            for cluster_id in set(labels):
                if cluster_id == -1:  # Noise
                    continue
                cluster_items = [
                    tech_debt_items[i] for i, label in enumerate(labels) if label == cluster_id
                ]
    ...
    async def _get_all_work_items(self, project_key: str | None = None) -> list[dict[str, Any]]:
        ...
        scroll_result = await self.qdrant_client.client.scroll(
            collection_name="jirascope_work_items", scroll_filter=scroll_filter, limit=10000
        )
    ...
    def _generate_labeling_suggestions(
        ...
        if len(label_counter) > 50:
            suggestions["optimizations"].append(
                "Large number of labels - consider label taxonomy review"
            )

        if not component_counter:
            suggestions["optimizations"].append(
                "No components used - consider adding component structure"
            )
