...
from ..analysis.content_analyzer import ContentAnalyzer, BatchContentAnalyzer
    ...
    async def fetch_and_process():
        ...
        if do_incremental:
            ...
            if not last_sync:
                ...
                click.echo(f"Processed: {result.processed_items}, Skipped: {result.skipped_items}, Failed: {result.failed_items}")
        ...
        if not do_incremental:
            ...
            if all_items:
                ...
                click.echo(f"Embedding results: {result.processed_items} processed, {result.failed_items} failed")
    ...
    async def search_items():
        async with LMStudioClient(config) as lm_client:
            async with QdrantVectorClient(config) as qdrant_client:
                ...
                results = await qdrant_client.search_similar_work_items(
                    embeddings[0], limit=limit
                )
                ...
                for i, result in enumerate(results, 1):
                    ...
                    click.echo(f"   Score: {score:.3f} | Type: {work_item['issue_type']} | Status: {work_item['status']}")
                    if work_item.get('description'):
                        desc = work_item['description'][:100] + "..." if len(work_item['description']) > 100 else work_item['description']
    ...
    async def run_validation():
        ...
        if report.results:
            ...
            for result in report.results[:5]:  # Show first 5
                ...
                click.echo(f"  {status} {result['query']} (score: {result.get('avg_similarity', 0):.3f})")
