...
class CostOptimizer:
    ...
    async def _get_service_breakdown(self, cost_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        embedding_cost = sum(op.get("cost", 0.0) for op in cost_data.get("embedding_operations", []))
    ...
    async def _identify_cost_optimizations(self, cost_data: Dict[str, Any]) -> List[OptimizationSuggestion]:
        ...
        if usage_analysis.caching_opportunities.get("potential_savings", 0.0) > 5.0:
            optimizations.append(OptimizationSuggestion(
                ...
                potential_savings=usage_analysis.caching_opportunities.get("potential_savings", 0.0),
                risk_level="low",
    ...
    async def _calculate_cost_per_analysis_type(self, cost_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    async def _calculate_efficiency_metrics(self, cost_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_input_tokens = sum(call.get("details", {}).get("input_tokens", 0) for call in claude_calls)
        total_output_tokens = sum(call.get("details", {}).get("output_tokens", 0) for call in claude_calls)
        total_tokens = total_input_tokens + total_output_tokens
        
        if total_tokens > 0:
            metrics["cost_per_token"] = total_cost / total_tokens
            metrics["token_efficiency_score"] = total_output_tokens / (total_input_tokens + 1)  # Avoid division by 0
...
class UsagePatternAnalyzer:
    ...
    async def _analyze_prompt_efficiency(self, usage_data: List[APICall]) -> PromptEfficiencyAnalysis:
        ...
        for category, calls in prompt_categories.items():
            ...
            potential_savings = sum(call.cost * 0.2 for call in expensive_calls) * 30  # Monthly estimate
            
            efficiency_results.append(PromptCategoryAnalysis(
                category=category,
        ...
        return PromptEfficiencyAnalysis(
            categories=efficiency_results,
            overall_efficiency_score=overall_efficiency_score
        )
    
    async def _identify_batch_opportunities(self, usage_data: List[APICall]) -> BatchOptimizationSuggestions:
        ...
        return BatchOptimizationSuggestions(
            ...
            estimated_total_savings=total_potential_savings
        )
    ...
    async def _identify_caching_opportunities(self, usage_data: List[APICall]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": "Implement results caching" if estimated_savings > 5.0 else "No significant caching opportunity"
        }
    ...
    def _calculate_cost_per_analysis_type(self, usage_data: List[APICall]) -> Dict[str, float]:
        ...
        for call in usage_data:
            operation_type = call.operation.split(".")[0] if "." in call.operation else call.operation
    ...
    async def _identify_batchable_claude_operations(self, usage_data: List[APICall]) -> Dict[str, Any]:
        ...
        for op_name, op_data in operations.items():
            ...
            if batch_count > 0 and batchable_calls > 0:
                ...
                op_data["suggested_batch_size"] = min(10, int(avg_batch_size * 1.5))  # Suggest slightly larger batch
