...
from .cost_optimizer import ComprehensiveCostReport, CostOptimizer, CostPredictions
...
class AdvancedCostReporter:
    ...
    async def generate_comprehensive_cost_report(self, period: str = "month") -> AdvancedCostReport:
        ...
        # Save report to file
        report_file = (
            self.report_path / f"cost_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    async def generate_daily_cost_summary(self) -> dict[str, Any]:
        ...
        avg_daily_cost = (
            sum(recent_daily_costs) / len(recent_daily_costs) if recent_daily_costs else 0
        )
        ...
        return {
            ...
            "vs_average": (
                ((today_costs - avg_daily_cost) / avg_daily_cost) * 100 if avg_daily_cost > 0 else 0
            ),
        }
    ...
    async def generate_monthly_cost_projection(self) -> dict[str, Any]:
        ...
        budget_percentage = (
            (projected_month_total / monthly_budget) * 100 if monthly_budget > 0 else 0
        )
    ...
    async def generate_cost_trend_analysis(self, days: int = 90) -> dict[str, Any]:
        ...
        for date in date_range:
            ...
            daily_costs.append({"date": date.isoformat(), "cost": cost})
        ...
        for item in daily_costs:
            ...
            if month_key not in month_data:
                month_data[month_key] = {"month": month_key, "costs": [], "total": 0}
        ...
        return {
            ...
            "trends": {"daily": daily_trend, "weekly": weekly_trend, "monthly": monthly_trend},
    ...
    async def generate_service_comparison_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "highest_cost_service": (
                max(service_costs.items(), key=lambda x: x[1])[0] if service_costs else None
            ),
            "cost_ratio": {
                "claude_to_embedding": (
                    service_costs.get("claude_api", 0)
                    / service_costs.get("embedding_processing", 1)
                    if service_costs.get("embedding_processing", 0) > 0
                    else 0
                )
            },
        }
    ...
    async def generate_cost_anomaly_report(self) -> dict[str, Any]:
        ...
        baseline_std = (
            sum((cost - baseline_avg) ** 2 for cost in daily_costs[:-1]) / (len(daily_costs) - 1)
        ) ** 0.5
        ...
        return {
            ...
            "percent_difference": (
                ((latest_cost - baseline_avg) / baseline_avg) * 100 if baseline_avg > 0 else 0
            ),
    ...
    async def _analyze_api_usage(self) -> ApiUsageBreakdown:
        ...
        services = {
            ...
            "embedding_processing": {"vector_generation": 40.0, "batch_processing": 5.0},
            "vector_storage": {"storage_fees": 2.0, "query_costs": 3.0},
        }
        ...
        return ApiUsageBreakdown(services=services, top_operations=top_operations, trends=trends)

    async def _analyze_cost_efficiency(
        self, base_report: ComprehensiveCostReport
        ...
        risk_assessment = {
            ...
            "budget_overrun_probability": (
                min(100, (total_cost / self.budget.monthly_budget) * 100)
                if self.budget.monthly_budget > 0
                else 0
            ),
            "largest_cost_factor": (
                max(base_report.service_breakdown.items(), key=lambda x: x[1])[0]
                if base_report.service_breakdown
                else None
            ),
            "optimization_priority": (
                "high" if potential_savings > 50 else "medium" if potential_savings > 20 else "low"
            ),
        }
    ...
    async def _check_budget_status(self) -> dict[str, Any]:
        ...
        # Calculate percentages
        daily_percent = (
            (daily_cost / self.budget.daily_budget) * 100 if self.budget.daily_budget > 0 else 0
        )
        monthly_percent = (
            (monthly_cost / self.budget.monthly_budget) * 100
            if self.budget.monthly_budget > 0
            else 0
        )
        ...
        monthly_status = (
            "over_budget" if monthly_cost > self.budget.monthly_budget else "under_budget"
        )
        ...
        return {
            ...
            "days_until_budget_exhausted": self._calculate_days_until_budget_exhausted(
                monthly_cost, monthly_percent
            ),
        }

    def _calculate_days_until_budget_exhausted(
        self, monthly_cost: float, monthly_percent: float
    ) -> int:
    ...
    async def _get_daily_costs(self, period: str = "month") -> list[dict[str, Any]]:
        ...
        while current_date <= end_date:
            ...
            daily_costs.append({"date": current_date.isoformat(), "cost": cost})
            current_date += timedelta(days=1)
    ...
    async def _get_costs_for_period(
