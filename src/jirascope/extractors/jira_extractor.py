...
from ..models import WorkItem, EpicHierarchy, EpicTree, ExtractionCost
...
class JiraExtractor:
            ...
            async with MCPClient(self.config) as client:
                ...
                for epic in epics:
                    try:
                        ...
                        logger.error(f"Failed to extract hierarchy for epic {epic.key}", error=str(e))
                        continue
    ...
    async def get_epic_tree(self, epic_key: str) -> EpicTree:
        ...
        try:
            async with MCPClient(self.config) as client:
                ...
                completion_percentage = self._calculate_completion_percentage([epic] + all_descendants)
    ...
    async def get_incremental_updates(
        ...
        project_key: str,
        last_sync: str,
        tracked_epics: Set[str],
        tracked_items: Set[str]
    ) -> List[WorkItem]:
    ...
    def _calculate_completion_percentage(self, items: List[WorkItem]) -> float:
        ...
        completed_items = sum(
            1 for item in items 
            if item.status.lower() in completed_statuses
        )
