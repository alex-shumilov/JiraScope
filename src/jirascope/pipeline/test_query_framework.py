    ...
    def add_test(self, test: RagTestQuery):
        ...
        if test.category not in self.categories:
            self.categories[test.category] = TestCategory(
                name=test.category,
                description=f"{test.category.capitalize()} tests",
                tests=[]
            )
...
class TestQueryManager:
    ...
    async def update_expected_results(self, jira_client: MCPClient, limit: int = 5):
        ...
        tests_to_update = [test for test in self.collection.get_all_tests() if not test.expected_work_items]
            ...
            try:
                ...
                if search_keywords:
                    ...
                    if expected_keys:
                        ...
                        logger.debug(f"Updated test {test.id} with {len(expected_keys)} expected results")
                    else:
