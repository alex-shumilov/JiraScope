...
class ComprehensiveQualityTester:
    ...
    async def run_full_quality_assessment(self, 
                                         test_plan: Optional[QualityTestPlan] = None) -> FullQualityReport:
    ...
    def _calculate_health_score(self, report: FullQualityReport) -> float:
        ...
        if report.embedding_consistency:
            ...
            score_components.append(("embedding_consistency", consistency_score * weights["embedding_consistency"]))
        
        # Analysis Reproducibility (0-1 scale)
        if report.analysis_reproducibility:
            reproducibility_score = report.analysis_reproducibility.get("reproducibility_score", 0.0)
            score_components.append(("analysis_reproducibility", reproducibility_score * weights["analysis_reproducibility"]))
    ...
    def _generate_recommendations(self, report: FullQualityReport) -> List[str]:
        ...
        if report.rag_quality:
            ...
            if f1_score < 0.5:
                recommendations.append("Critical: RAG quality is poor (F1 < 0.5). Consider re-training embeddings or improving test queries.")
            elif f1_score < 0.7:
                recommendations.append("Moderate: RAG quality needs improvement (F1 < 0.7). Review semantic search implementation.")
            
            # Check individual test categories
            failed_by_category = {}
            if report.rag_quality.test_results:
                for result in report.rag_quality.test_results:
                    query = next((q for q in self.test_manager.get_all_tests() if q.id == result.query_id), None)
                ...
                for category, count in failed_by_category.items():
                    if count > 1:
                        recommendations.append(f"Review {category} category tests: {count} tests failing with low F1 scores.")
        
        # Embedding Consistency recommendations
        if report.embedding_consistency:
            consistency = report.embedding_consistency.get("overall_consistency", 0.0)
            if consistency < 0.95:
                recommendations.append("Critical: Embedding consistency is poor. Check embedding service stability.")
            elif consistency < 0.99:
                recommendations.append("Moderate: Embedding consistency needs improvement. Consider fixed seeds or model versioning.")
        
        # Reproducibility recommendations
        if report.analysis_reproducibility:
            reproducibility = report.analysis_reproducibility.get("reproducibility_score", 0.0)
            if reproducibility < 0.9:
                recommendations.append("Critical: Analysis results are not reproducible. Check for non-deterministic components.")
        
        # Regression recommendations
        if report.performance_regression and report.performance_regression.get("has_regression", False):
            change = report.performance_regression.get("change", 0.0)
            recommendations.append(f"Regression detected: F1 score decreased by {abs(change):.2f}. Review recent changes.")
        
        # General recommendations
        if report.overall_health_score < 60:
            recommendations.append("General: System quality is poor. Consider a comprehensive review of embedding and RAG components.")
        elif report.overall_health_score < 75:
            recommendations.append("General: System quality is acceptable but could be improved. Focus on specific failing tests.")
    ...
    async def generate_quality_trend_report(self) -> Dict[str, Any]:
        ...
        for report in self.previous_reports:
            ...
            consistency_scores.append(embedding_consistency.get("overall_consistency", 0.0) if embedding_consistency else 0.0)
