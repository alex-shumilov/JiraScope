...
class IncrementalProcessor:
    ...
    async def process_incremental_updates(
        ...
        logger.info(f"Processing incremental updates: {len(new_items)} new, {len(updated_items)} updated")
        ...
        try:
            ...
            if not items_to_process:
                ...
                return ProcessingResult(
                    skipped_items=len(updated_items),
    ...
    def get_tracked_items(self, project_key: Optional[str] = None) -> Set[str]:
        ...
        if project_key:
            return {
                key for key, data in tracked_items.items()
    ...
    def _update_tracking_data(
        ...
        for item in items:
            ...
            project_items = sum(
                1 for data in tracked_items.values() 
                if data.get("project_key") == project_key
            )
