...
class QualityTestSuite:
    ...
    async def create_custom_test_from_epic(
...
class RAGQualityTester:
    ...
    async def run_quality_tests(self) -> RAGQualityReport:
        ...
        try:
                ...
                async with QdrantVectorClient(self.config) as qdrant_client:
                    for test_query in self.test_queries:
                        try:
                            ...
                            embeddings = await lm_client.generate_embeddings(
                                [test_query.query_text]
                            )
                            if not embeddings:
                                raise ValueError(
                                    f"Failed to generate embeddings for query: {test_query.query_text}"
                                )

                            # Search for similar work items
                            search_results = await qdrant_client.search_similar_work_items(
                                embeddings[0], limit=10, score_threshold=0.3
                            )
                            ...
                            found_set = set(
                                result["work_item"]["key"] for result in search_results[:5]
                            )

                            precision = (
                                len(expected_set & found_set) / len(found_set) if found_set else 0
                            )
                            recall = (
                                len(expected_set & found_set) / len(expected_set)
                                if expected_set
                                else 0
                            )
                            f1_score = (
                            ...
                            min_similarity_met = (
                                all(
                                    ...
                                    if result["work_item"]["key"] in expected_set
                                )
            ...
            if not self.baseline_results or overall_f1 > self.baseline_results.get(
                "overall_f1_score", 0
            ):
                self.baseline_results = {
                    "overall_f1_score": overall_f1,
                ...
                if len(embeddings) >= 2:
                    ...
                    consistency_results.append(
                        EmbeddingConsistencyResult(
                            ...
                            > 0.99,  # Very high threshold for consistency
                        )
                    )

        # Calculate overall consistency
        if consistency_results:
            overall_consistency = sum(r.average_similarity for r in consistency_results) / len(
                consistency_results
            )
