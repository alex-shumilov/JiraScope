...
class EmbeddingQualityValidator:
    ...
    async def validate_embedding_quality(self) -> QualityReport:
        ...
        logger.info(f"Starting embedding quality validation with {len(self.test_queries)} test queries")
                ...
                async with QdrantVectorClient(self.config) as qdrant_client:
                    ...
                    for i, query in enumerate(self.test_queries):
                        try:
                            ...
                            logger.debug(f"Test query {i+1}/{len(self.test_queries)}: {'PASS' if result['passed'] else 'FAIL'}")
    ...
    def _generate_recommendations(self, results: List[Dict[str, Any]], overall_score: float) -> List[str]:
        """Generate improvement recommendations based on test results."""
        recommendations = []
        
        if overall_score < 50.0:
            recommendations.append("Overall quality is poor - consider re-generating embeddings with different parameters")
        ...
        if no_result_queries:
            recommendations.append(f"{len(no_result_queries)} queries returned no results - consider adding more diverse work items")
        ...
        if low_similarity_queries:
            recommendations.append(f"{len(low_similarity_queries)} queries had low similarity scores - embedding model may need tuning")
        ...
        if total_results < len(self.test_queries) * 2:  # Less than 2 results per query on average
            recommendations.append("Limited search results suggest sparse data - consider adding more work items")
            ...
            async with LMStudioClient(self.config) as lm_client:
                async with QdrantVectorClient(self.config) as qdrant_client:
                    ...
                    for query in test_queries:
                        ...
                        await qdrant_client.search_similar_work_items(
                            query_embeddings[0],
                            limit=10
                        )
