...
from jirascope.analysis.content_analyzer import (
    BatchContentAnalyzer,
    ContentAnalyzer,
    QualityAnalysisPrompts,
)
from jirascope.core.config import Config
from jirascope.models import BatchAnalysisResult, QualityAnalysis, SplitAnalysis, SplitSuggestion
...
class TestQualityAnalysisPrompts:
    ...
    def test_description_quality_prompt_formatting(self):
        ...
        prompt = QualityAnalysisPrompts.DESCRIPTION_QUALITY_PROMPT.format(
            summary="Test Story", issue_type="Story", description="Test description"
        )
    ...
    def test_batch_quality_prompt_formatting(self):
        ...
        prompt = QualityAnalysisPrompts.BATCH_QUALITY_PROMPT.format(work_items=work_items_text)
...
class TestContentAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_analyze_description_quality_success(
        self, mock_config, mock_claude_client, sample_work_items
    ):
        """Test successful description quality analysis."""
        with patch(
            "jirascope.analysis.content_analyzer.ClaudeClient", return_value=mock_claude_client
        ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_description_quality_with_malformed_json(
        self, mock_config, sample_work_items
    ):
        """Test handling of malformed JSON response from Claude."""
        claude_client = AsyncMock()
        claude_client.analyze.return_value = AsyncMock(
            content="Malformed JSON response without proper structure", cost=0.02
        )
    ...
    @pytest.mark.asyncio
    async def test_suggest_work_item_splits_should_split(
        self, mock_config, mock_claude_client, sample_work_items
    ):
    ...
    async def test_context_manager_cleanup(self, mock_config):
        ...
        with patch("jirascope.analysis.content_analyzer.ClaudeClient") as mock_claude:
            ...
            async with ContentAnalyzer(mock_config):
                pass
...
class TestBatchContentAnalyzer:
    ...
    def mock_claude_client(self, mock_claude_responses):
        ...
        claude_client.analyze.return_value = AsyncMock(
            content=json.dumps(batch_response), cost=0.05
        )

        return claude_client

    @pytest.mark.asyncio
    async def test_analyze_multiple_items_success(
        self, mock_config, mock_claude_client, sample_work_items
    ):
        ...
        with patch(
            ...
            async with BatchContentAnalyzer(mock_config) as batch_analyzer:
                ...
                result = await batch_analyzer.analyze_multiple_items(
                    work_items=work_items, analysis_types=["quality"], batch_size=2
                )
    ...
    @pytest.mark.asyncio
    async def test_analyze_multiple_items_with_batch_size(
        self, mock_config, mock_claude_client, sample_work_items
    ):
    ...
    @pytest.mark.asyncio
    async def test_batch_quality_analysis_json_parsing_failure(
        self, mock_config, sample_work_items
    ):
        """Test batch analysis with JSON parsing failure."""
        claude_client = AsyncMock()
        claude_client.analyze.return_value = AsyncMock(content="Malformed JSON response", cost=0.03)
