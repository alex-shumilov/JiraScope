...
from jirascope.analysis.temporal_analyzer import ScopeDriftDetector, TemporalAnalyzer
from jirascope.core.config import Config
from jirascope.models import BatchAnalysisResult, ScopeDriftAnalysis, ScopeDriftEvent
...
class TestScopeDriftDetector:
    ...
    async def test_calculate_semantic_similarity(self):
        ...
        async with self.detector:
            ...
            self.detector.lm_client.generate_embeddings = AsyncMock(
...
class TestTemporalAnalyzer:
    ...
    async def test_analyze_scope_drift_success(self, mock_config, mock_clients, sample_work_items):
        ...
        with (
            patch("jirascope.analysis.temporal_analyzer.MCPClient", return_value=jira_client),
    ...
    @pytest.mark.asyncio
    async def test_analyze_scope_drift_no_history(
    ...
    @pytest.mark.asyncio
    async def test_detect_scope_drift_for_project_success(
        self, mock_config, mock_clients, sample_work_items
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_scope_drift_with_time_range(
        self, mock_config, mock_clients, sample_work_items
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_claude_failure(
        self, mock_config, mock_clients, sample_work_items
    ):
