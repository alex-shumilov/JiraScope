...
@pytest.fixture
def mock_config():
    return Config(jira_mcp_endpoint="http://test.com")


@pytest.fixture
def sample_test_queries():
    return ["authentication and login", "database migration", "user interface"]
...
async def test_validate_embedding_quality_success(mock_config, sample_test_queries):
    ...
    mock_search_results = [
        {
            "score": 0.8,
            "work_item": {"key": "PROJ-1", "summary": "Login functionality", "issue_type": "Story"},
        },
        {
            "score": 0.7,
            "work_item": {"key": "PROJ-2", "summary": "User authentication", "issue_type": "Task"},
        },
    ]
    ...
    mock_search_results = [
        {
            ...
            "work_item": {"key": "PROJ-1", "summary": "Unrelated item", "issue_type": "Bug"},
        }
    ]
...
async def test_test_single_query(mock_config):
    ...
    result = await validator._test_single_query(
        "user authentication", mock_lm_client, mock_qdrant_client
    )
...
def test_generate_recommendations():
    ...
    assert any(
        "good" in rec.lower() or "no specific improvements needed" in rec.lower()
