...
async def test_run_quality_tests_success(mock_config, sample_test_queries):
    ...
    db_mock_results = [
        ...
        {
            "score": 0.7,
            "work_item": {"key": "PROJ-012", "summary": "Schema updates", "issue_type": "Task"},
        },
    ]
...
async def test_validate_analysis_consistency(mock_config):
    ...
    tester._get_test_work_items = AsyncMock(
        return_value=[
            AsyncMock(key="TEST-1", summary="Test item 1", description="Test description 1"),
            AsyncMock(key="TEST-2", summary="Test item 2", description="Test description 2"),
        ]
    )
...
async def test_benchmark_embedding_performance(mock_config):
    ...
    tester._get_test_work_items = AsyncMock(
        return_value=[
            AsyncMock(
                key=f"TEST-{i}", summary=f"Test item {i}", description=f"Test description {i}"
            )
